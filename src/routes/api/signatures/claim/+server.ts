...
export async function POST({ request, getClientAddress }: RequestEvent) {
    ...
    try {
        const existing = await db.select().from(signatures).where(eq(signatures.text, text.toLowerCase())).get();
        ...
        const userRecentClaims = recentClaims.filter(s =>
            s.ipHash === ipHash &&
            (Date.now() - s.createdAt.getTime()) < ONE_HOUR
        );
